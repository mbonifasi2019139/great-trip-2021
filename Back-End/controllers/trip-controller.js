...
function getTouristsPlacesTrips(req,res){
    let tripId = req.params.idT;

    Trip.findById(tripId).populate("tourists_places").exec((err,tripFinded)=>{
        if(err){
            return res.status(500).send({message: "Error en el servidor"});
        }else if(tripFinded){
            let touristsPlaces = tripFinded.tourists_places;
            return res.send({message: "Viajes", touristsPlaces});
        }else{
